-- Тестовая схема с table inheritance
...
-- Родительская таблица для задач
...
CREATE TABLE goals (
    ...
    CONSTRAINT end_date_required CHECK (end_date IS NOT NULL)
) INHERITS (items_base);
...
CREATE VIEW habits_calc with (security_invoker = on) AS (
    ...
    SELECT *,
        -- Плановое значение на дату с начала учета
        --NULL::DECIMAL AS plan_value,
        -- Среднее изменение в день с начала учета
        fact_change / NULLIF(date - begin_date + 1, 0) AS avg_change,
        -- Среднее значение в день с начала интервала
        ...
        -- (target_value - fact_value) / NULLIF(remaining_duration - (value IS NOT NULL)::INTEGER, 0) AS daily_target_change,
        -- Процент выполнения цели
...
-- Политики для goals (наследуют, но для clarity)
DROP POLICY IF EXISTS "Users can view own goals" ON goals;
CREATE POLICY "Users can view own goals" ON goals
    FOR SELECT USING (user_id = (auth.jwt() ->> 'sub')::uuid);

-- Аналогично для habits
